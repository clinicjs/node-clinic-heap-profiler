...
test('cmd - test collect - data exists, html generated', function (t) {
  const tool = new ClinicHeapProfilder()
  ...
  tool.collect([process.execPath, path.join('test', 'fixtures', 'inspect.js')], function (err, dirname) {
    ...
    tool.visualize(dirname, htmlName, function (err) {
      ...
      fs.readFile(htmlName, function (err, content) {
        ...
        tool.visualize(dirname, htmlName, function () {
...
test('cmd - test visualization - missing data', function (t) {
  const tool = new ClinicHeapProfilder({ debug: true })

  tool.visualize('missing.clinic-flame', 'missing.clinic-flame.html', function (err) {
...
test('cmd - test collect - does not crash on webassembly frames', function (t) {
  ...
  tool.collect([process.execPath, path.join('test', 'fixtures', 'wasm.js')], function (err, dirname) {
    if (err) return cleanup(err, dirname)

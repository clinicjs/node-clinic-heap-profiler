...
class History extends EventEmitter {
  ...
  setData (dataTree) {
    ...
    this.availableExclusions = ['is:init', 'is:inlinable']
  ...
  serializeExcludes (exclude) {
    ...
    const bits = this.availableExclusions.map(name => (exclude.has(name) ? '1' : '0')).join('')
    ...
    for (let i = 0; i < bits.length; i += groupSize) {
      ...
      const word = bits.slice(i, i + groupSize).padEnd(groupSize, '0')
  ...
  deserializeExcludes (string) {
    ...
    groups.forEach((group, i) => {
      const groupBits = parseInt(group, 16)
        .toString(2)

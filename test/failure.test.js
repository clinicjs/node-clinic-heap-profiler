...
test('cmd - test collect - works with nonzero exit code', function (t) {
  ...
  tool.collect([process.execPath, '-e', 'throw new Error()'], function (err, dirname) {
...
test(
  'cmd - test collect - errors on SIGKILL',
  {
  ...
  function (t) {
    ...
    tool.collect([process.execPath, '-e', 'process.kill(process.pid, 9)'], function (err, dirname) {
      t.ok(err)
